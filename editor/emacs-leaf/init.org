...
- TODO: use the modern keymap-set
  https://pxaka.tokyo/blog/2025/0222-replace-define-key-with-keymap-set/
- TODO: use the modern advice
...
(eval-and-compile
  ...
  ;; Single package missing results in re-downloads of the entire archive.
  ;; Trust the cache for a day instead.
  (defvar toy/package-refresh-interval (* 24 60 60))
  (defun toy/package-refresh-throttle (orig &rest args)
    (let ((f (expand-file-name "archives/melpa/archive-contents" package-user-dir)))
      (if (and (not (called-interactively-p 'any))
               (file-exists-p f)
               (< (float-time (time-since (file-attribute-modification-time
                                           (file-attributes f))))
                  toy/package-refresh-interval))
          (message "package-refresh-contents: skipped (cache is fresh)")
        (apply orig args))))
  (advice-add 'package-refresh-contents :around #'toy/package-refresh-throttle)
...
#+BEGIN_SRC elisp
(when (display-graphic-p)
  (set-frame-font "IntoneMono Nerd Font")
  (set-face-attribute 'default nil :family "IntoneMono Nerd Font" :height 135)
...
(when (fboundp 'neomacs-effect-schema)
  ...
  (setopt neomacs-window-animations-off t
          neomacs-cursor-motion-enabled nil
          neomacs-cursor-size-transition-enabled nil
          neomacs-buffer-transition-enabled nil
          neomacs-scroll-transition-enabled nil)
  (setopt neomacs-effect-cursor-color-cycle '(:enabled nil)))
#+END_SRC

** Window divider

Delete the border characers
...
#+BEGIN_SRC elisp
(setopt auto-revert-interval 3.0
...
(leaf evil-surround
  ...
  ;;   :config
  ;;   (keymap-set evil-outer-text-objects-map evil-textobj-entire-key 'evil-entire-entire-buffer)
  ;;   (keymap-set evil-inner-text-objects-map evil-textobj-entire-key 'evil-entire-entire-buffer))
...
(leaf evil-collection
  ...
  (with-eval-after-load 'magit
    (evil-define-key 'normal magit-mode-map
      ...
      "zm" #'magit-section-show-level-2
      "C-c C-u" #'magit-section-up
...
(leaf auto-package-update
  ...
  :init
  ;; `auto-package-update-maybe' blocks startup once the interval has passed
  (add-hook 'emacs-startup-hook
            (lambda () (run-with-idle-timer 5 nil #'auto-package-update-maybe))))
#+END_SRC
...
(leaf diff-hl
  ...
  (defun toy/diff-hl-refresh-on-focus ()
    (when (frame-focus-state)
      (dolist (win (window-list-1 nil 'nomini t))
        (with-current-buffer (window-buffer win)
          (when (and buffer-file-name diff-hl-mode)
            (vc-refresh-state)
            (diff-hl-update))))))
  (add-function :after after-focus-change-function
                #'toy/diff-hl-refresh-on-focus)
  :hook
...
(leaf doom-modeline
  ...
  :after nerd-icons
  :custom
  (;; (doom-modeline-icon . t) ;; why error here?
...
(leaf neotree
  ...
  ((neo-theme . 'nerd-icons)
   (neo-confirm-create-file . #'always)
   (neo-confirm-create-directory . #'always)
   (neo-window-position quote right)
   (neo-window-width . toy/sidebar-width)
   (neo-window-fixed-size)
   (neo-show-hidden-files . t)
   (whitespace-global-modes . '(not neotree-mode)))
  ...
  ;; Update cursor position after up-node
  (advice-add 'neotree-select-up-node :after
              (lambda (&rest _)
                (evil-first-non-blank)))

  ;; Fix: `read-file-name-function' implementations (e.g. consult) may visit
  ;; the selected file and switch the current buffer. Use `read-file-name-default'
  ;; since we are specifying a path to create, not opening an existing file.
  ;; Also ensure buffer operations run in the neotree buffer.
    ...
    (catch 'rlt
      (let ((is-file nil))
        ...
        (when (file-exists-p filename)
          ...
          (throw 'rlt nil))
        (when (and is-file
                   (funcall neo-confirm-create-file
                            (format "Do you want to create file %S ?" filename)))
        ...
        (when (and (not is-file)
                   (funcall neo-confirm-create-directory
                            (format "Do you want to create directory %S?" filename)))
          (mkdir filename t)
          (with-current-buffer (neo-global--get-buffer)
            (neo-buffer--save-cursor-pos filename)
            (neo-buffer--refresh nil)))))))
...
(leaf emmet-mode
  ...
  ((html-mode-hook . emmet-mode)
   ...
   (javascript-mode-hook . emmet-mode)
...
(leaf magit
  ...
  ;; Side-by-side magit blame
  ;; https://github.com/magit/magit/issues/2717#issuecomment-2165837070
  ;; (setq magit-blame-styles  
  ;;        '((margin  
  ;;           (margin-width . 42)  
  ;;           (margin-format . ("%C %c %s %f"))  ; %s moved to end (before %f)  
      ...
      (let ((orig-result (funcall old-fn cmd test?)))
        (if (and (not test?)                             ;; for check lsp-server-present?
            ...
            (progn
              ...
              ;; The default `--json-object-type' is `plist', which must match `lsp-use-plists':
              (append (list "emacs-lsp-booster"
                            "--json-object-type" (if (bound-and-true-p lsp-use-plists) "plist" "hashtable")
                            "--")
                      orig-result) )
          orig-result)))
...
(defun toy/nearest-marker (files)
  "Deepest ancestor directory of the current file holding one of FILES."
  (when buffer-file-name
    (car (sort (delq nil
                     (mapcar (lambda (f) (locate-dominating-file buffer-file-name f))
                             files))
               (lambda (a b) (> (length a) (length b)))))))

;; Format on save with biome. TODO: support prettier/eslint
;; https://zenn.dev/craneduck/articles/3c0da51d4767d5
(leaf reformatter
  :config
  (reformatter-define biome-format
    ;; NOTE: It requires `biome` to be in path. Is it npm-compatible?
    :program "biome"
    :args `("format" "--stdin-file-path" ,(buffer-file-name))
    :lighter " BiomeFmt")

  (defun toy/biome-format-on-save-maybe ()
    (when (toy/nearest-marker '("biome.json" "biome.jsonc"))
      (biome-format-on-save-mode)))

  (add-hook 'typescript-mode-hook 'toy/biome-format-on-save-maybe)
  (add-hook 'js-mode-hook 'toy/biome-format-on-save-maybe))
#+END_SRC
...
(leaf lsp-ui-imenu
  ...
  (advice-add 'lsp-ui-imenu--visit :after
              (lambda (&rest _)
                (toy/smart-recenter))))
#+END_SRC
...
https://github.com/jscheid/prettier.el?tab=readme-ov-file#enabling-per-file--per-directory"
  ...
  ;; TODO: enable only when configuration file is detected
...
(leaf tree-sitter
  ...
  :init
  (add-hook 'emacs-startup-hook
            (lambda () (run-with-idle-timer 0.5 nil #'global-tree-sitter-mode)))
  :defer-config
...
(leaf typst-ts-mode
  :after lsp-mode
  ;; :vc (:url "https://github.com/meow_king/typst-ts-mode")
  :custom
...
(leaf typescript-mode
  ...
  :hook
  ((typescript-mode-hook . my/ts-hook))
  :config
  ;; TODO: clean it up, or use an existing utility
  (defun my/ts-deno-p ()
    (let ((deno (toy/nearest-marker '("deno.json" "deno.jsonc" "deno.lock")))
          (node (toy/nearest-marker '("package.json" "node_modules"))))
      (and deno (or (null node) (>= (length deno) (length node))))))

  (defun my/ts--clients-matching (regexp)
    (seq-filter (lambda (id) (string-match-p regexp (symbol-name id)))
  ...
  (defun my/ts-hook ()
    ...
    (setq-local lsp-disabled-clients
                (append lsp-disabled-clients
                        (if (my/ts-deno-p)
                            (append '(ts-ls eslint) (my/ts--clients-matching "biome"))
                          '(deno-ls))
                        (unless (toy/nearest-marker '("biome.json" "biome.jsonc"))
                          (my/ts--clients-matching "biome"))))
    (lsp-deferred)))
#+END_SRC
...
(leaf consult
  ...
  ;;   (defmacro my/with-abort-minibuffer (&rest body)
  ;;     "Utility function for unwinding the minibuffer and calling a function, while not nesting a call
  ;; stack, keeping `this-comman' clean."
  ;;     `(if (minibufferp)
  ;;          (progn
  ...
  ;;         (my/with-abort-minibuffer
  ;;          (consult-find-file-with-preview "Find file: " "/" nil nil dir))))
  ...
  (defun prt ()
    (interactive)
    (message "%s" embark--command))
  ;; (keymap-set minibuffer-mode-map "C-c e" #'prt)

  (progn ;; `find-file' via `embark-become' with CWD text
    (defun my-embark-become-prepend-default-dir (orig-fun command input)
      ...
      (if (and (memq command '(find-file find-file-other-window
          ...
          (funcall orig-fun command (expand-file-name input default-directory))
    ...
    ;;         (funcall orig-fun command (expand-file-name input dir)))
    ;;     (funcall orig-fun command input)))
...
(leaf embark
  ...
  (("C-h B" . embark-bindings)
   ("C-h K" . embark-bindings-in-keymap))
  :init
  ;; Somehow I have to put it in `:init'
...
*** Which-key integration with =embark=
...
Qiita Markdown support.

#+BEGIN_SRC elisp
(leaf ox-qmd
  :commands (org-qiita-export-buffer-to-book)
  :vc (:url "https://github.com/toyboot4e/ox-qmd")
  :ensure nil
  :config
...
** Minimum org-mode PDF output

Add the followings to the org file header:
...
(leaf ox-typst
  :vc (:url "https://github.com/toyboot4e/ox-typst")
  :ensure nil
  :after org)
#+END_SRC
...
(evil-define-key 'normal org-mode-map
  ...
  " oq" #'org-qiita-export-buffer-to-book)
#+END_SRC
...
(defun toy/disable-cursor-at-eof ()
  ...
  (unless (or (window-minibuffer-p) (<= (line-number-at-pos) 1))
    (when (= (point) (point-max))
      (unless (string-match-p (regexp-quote "SQL") (buffer-name))
        (forward-line -1)))))
...
(defun toy/smart-esc-tr (_)
  (if (toy/do-tr-esc) "ESC" "C-g"))
(keymap-set key-translation-map "ESC" #'toy/smart-esc-tr)
#+END_SRC
...
(keymap-set evil-normal-state-map "x" 'toy/null-x)
...
(keymap-set evil-normal-state-map "s" 'toy/null-s)
...
(keymap-set evil-ex-completion-map "C-f" 'forward-char)
(keymap-set evil-ex-completion-map "C-b" 'backward-char)
(keymap-set evil-ex-completion-map "C-a" 'move-beginning-of-line)
(keymap-set evil-ex-completion-map "C-e" 'move-end-of-line)

(keymap-set evil-ex-completion-map "C-d" 'evil-delete-char)
(keymap-set evil-ex-completion-map "C-h" 'evil-delete-backward-char)
(keymap-set evil-ex-completion-map "C-k" 'evil-delete-line)
(keymap-set evil-ex-completion-map "C-u" 'toy/backward-kill-line)
#+END_SRC
...
(evil-define-key 'replace 'global
  "\C-s" (_fn (evil-force-normal-state)
              (save-buffer)))
#+END_SRC
...
(defun toy/proj-find ()
  ...
  (interactive)
  (consult-fd (consult--project-root) ". "))
...
(evil-define-key 'normal 'toy/global-mode-map
  ...
  ;; " ff" (lambda () (interactive) (consult-find-file-with-preview default-directory))
  ;; " ff" (_cd find-file)
  ...
  ;;; Bookmarks
  " bb" #'my/consult-bookmark-with-tags
  (kbd "SPC b RET") #'my/bookmark-set
  " bt" #'my/bookmark-set-auto-tag
  " bf" #'bmkp-file-this-dir-jump ;; TODO: project jump
  " bF" #'bmkp-file-jump
...
(keymap-set minibuffer-local-map "C-a" #'evil-first-non-blank)
(keymap-set minibuffer-local-map "C-e" #'end-of-line)
(keymap-set minibuffer-local-map "C-r" #'evil-paste-from-register)

;; NOTE: The default key bindings for going up directory are `M-backspace' and `C-backspace', and
;; they don't work well. `M-backspace' is delete on macOS and `C-backspace' is interpreted as `C-h'
(keymap-set minibuffer-local-map "C-w" #'backward-kill-word)

(with-eval-after-load 'vertico
  ;; TODO: recenter-top-bottom for vertico buffer
  (keymap-set vertico-map "C-o" #'other-window)
  (keymap-set vertico-map "C-u" #'vertico-scroll-down)
  (keymap-set vertico-map "C-d" #'vertico-scroll-up)
  ;; TODO: use isearch forward?
  (keymap-set vertico-map "C-r" #'evil-paste-from-register))
#+END_SRC
...
#+BEGIN_SRC elisp
(with-eval-after-load 'evil
  (define-key evil-window-map (kbd "g d")
...
(leaf proced-narrow
  ...
  :config
  (keymap-set proced-mode-map "C-j" #'proced-narrow))
#+END_SRC
...
(leaf which-key
  ...
  :config
  (keymap-set help-map "M" 'which-key-show-major-mode)
  (which-key-mode))
#+END_SRC
...
(defun toy/setup-dark-theme ()
  ...
  (defun toy/set-transparent-background-on-tty ()
    (interactive)

    ;; Preserve backround color of these faces:
    (defconst my-bg-keep-faces
      '(region
        secondary-selection
        isearch
        lazy-highlight
        ...
        ;; TODO: ignore git gutter?
        ;; TODO: ignore magit hunks

        ;; diff-hl (TODO: regex match)
        diff-hl-insert
        diff-hl-delete
        diff-hl-change
        diff-hl-unknown
        ...
        magit-diff-added-refined
        magit-diff-removed-refined
        magit-diff-context
    ...
    ;; [1] frame / default face anchor
    (defun my-strip-bg (&rest _)
      (mapc
       (lambda (face)
         (when (and (facep face)
                    (not (memq face my-bg-keep-faces)))
           (set-face-attribute face nil :background 'unspecified)))
       (face-list)))

    ;; [2] Keep default background transparent (per-face pollution cleanup)
    (defun my-fix-default-bg (&rest _)
      (set-face-background 'default "unspecified-bg" (selected-frame)))

    (my-fix-default-bg)
    (add-hook 'after-load-theme-hook #'my-fix-default-bg)

    (advice-add 'set-face-background :after
                (lambda (face &rest _)
                  (when (eq face 'default)
                    (my-fix-default-bg))))

    (add-hook 'after-load-theme-hook #'my-strip-bg)
    (add-hook 'after-change-major-mode-hook #'my-strip-bg)

    ;; Set vertico color
    (set-face-attribute 'vertico-current nil
                        :background "#448888" ;; TODO: Choose better color
                        :extend t))

  (toy/set-transparent-background-on-tty))
#+END_SRC
