...
(leaf bookmark+
  ...
  ((bookmark-save-flag . 1)
   ...
   (bookmark-use-automatic-names . t)
   ;; (bmkp-propertize-bookmark-names-flag . t) ;; This is extremely slow
   ))
#+END_SRC
...
#+BEGIN_SRC elisp
(defvar my/bookmark-auto-tag nil)
#+END_SRC

Create a bookmark and set the auto tag:
...
(defun my/bookmark-set ()
  ...
  (let ((bookmark-name (my/get-default-bookmark-name))
        (tag-name (or my/bookmark-auto-tag (consult-bookmark-tags-with-default "Tag: " t))))
...
Change the auto tag with =consult=:

#+BEGIN_SRC elisp
(defun my/bookmark-set-auto-tag (new-tag)
  "Sets `my/bookmark-auto-tag' with `consult'."
  (interactive
   (list (consult-bookmark-tags-with-default "Tag: " t)))

  (setq my/bookmark-auto-tag (if (string= new-tag "") nil new-tag))
  (message "New auto tag: %s" my/bookmark-auto-tag))
#+END_SRC

*** =consult-bookmark-with-tags=

#+BEGIN_SRC elisp
;; Icons are embedded by directly calling `nerd-icons.el'
...
#+BEGIN_SRC elisp
;; (defvar my/pr-count 0)
;; (defvar my/pr-count-pr-timer nil)

;; (with-eval-after-load 'doom-modeline
;;   ;; Function to fetch PR count asynchronously (generated by AI)
;;   (defun my/pr-count-fetch-pr-count ()
;;     "Fetch GitHub PR count asynchronously."
;;     (let* ((buf (get-buffer-create " *gh-pr-count*"))
;;            (proc (start-process "gh-pr-count" buf
;;                                 "gh" "pr" "list"
...
;;                (setq my/pr-count count)
;;                (force-mode-line-update))))))))

;;   (defun my/pr-count-start-timer ()
;;     "Start/Stop the timer for GitHub PR count fetching."
;;     (when (timerp my/pr-count-pr-timer)
;;       (cancel-timer my/pr-count-pr-timer))
;;     (setq my/pr-count-pr-timer
;;           ;; updated every miniute
;;           (run-with-timer 0 60 #'my/pr-count-fetch-pr-count)))

;;   (my/pr-count-start-timer)

;;   (doom-modeline-def-segment github-pr-count
;;     "Display count of PRs requesting your review."
;;     (when (> my/pr-count 0)
;;       (concat (number-to-string my/pr-count))))

;;   ;; The 'main line with github-pr-count at the beginning
;;   (doom-modeline-def-modeline 'my-line
;;     '(github-pr-count eldoc bar window-state workspace-name window-number modals matches follow buffer-info remote-host buffer-position word-count parrot selection-info)
...
;;   ;; TODO: set target other modes than org-mode, too
;;   (add-to-list 'doom-modeline-mode-alist '(org-mode . my-line)))
#+END_SRC
...
(leaf reformatter
  ...
  (reformatter-define biome-format
    ;; NOTE: It requires `biome` to be in path
    :program "biome"
...
TODO: docstring treesitter

#+BEGIN_SRC elisp
(leaf msgu
  :ensure nil
  :vc (:url "https://github.com/jcs-elpa/msgu"))

(leaf ts-docstr
  :ensure nil
  :vc (:url "https://github.com/emacs-vs/ts-docstr")
  :after msgu)
#+END_SRC
